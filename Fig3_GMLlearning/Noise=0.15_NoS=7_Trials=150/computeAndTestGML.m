...
NoiseDir = 0.15; % Directional noise.
...
%% Compare the GML component in the symbol representation subspace with the one outside.
Prjs = GML'*[Symbols; Biases];
GMLin = [Symbols; Biases]*Prjs';
GMLout = GML-GMLin;

figure
%feather(norm(GMLin),norm(GMLout),'Color','r','LineWidth',0.75)
plot([0 1]*norm(GMLin),[0 1]*norm(GMLout),'r-','LineWidth',0.75)
hold on
plot([1 1]*norm(GMLin),[0 1]*norm(GMLout),'r--')
plot([0 1]*norm(GMLin),[1 1]*norm(GMLout),'r--')
set(gca,'TickDir','out','DataAspectRatio',[1 1 1])
xlabel('||GML_{in}||, prj. in item space')
ylabel('||GML_{out}||, prj. in outer space')
title(sprintf('||GML_{out}||/||GML|| = %.3g%%, cos(\\alpha) = %.3g',norm(GMLout)/norm(GML)*100,GMLout'*GML/(norm(GMLout)*norm(GML))))
